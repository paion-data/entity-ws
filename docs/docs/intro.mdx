...
- Feel free to change the values of `ENTITY_WS_ARANGO_USERNAME` and `ENTITY_WS_ARANGO_PASSWORD`. The values below are
examples only.
- Please do NOT, however, modify the values `http://arango-db:8529` and `$ENTITY_WS_ARANGO_PASSWORD`
...
There is a reference [docker-compose.yaml](https://github.com/paion-data/entity-ws/blob/master/docker-compose.yaml) where
